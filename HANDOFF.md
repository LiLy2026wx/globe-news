...
4. **点大省(新疆/西藏)又糊成金色** → 选中的省仍填 0.55 金色 + 镜头拉近 → 大省糊满屏。修：cn 视图选中省也不填充，改用**亮金描边**(`strokeColor()`)高亮；点省镜头 1.2→1.5（`7dac457`）。Playwright 点新疆/西藏验证通过。
5. **选一个省后点别的省一直卡住** → **真根因(Playwright 实测确认)：侧边面板(手机 88vw)盖住大半个地球，点东部省份其实点在面板上**，只有最西的新疆露在外面可点。修(`4ee7840`)：在面板里加**省份标签栏(chips，按热度排序)**，直接在面板内选省、不依赖去点被盖住的地图——412px 手机视口实测点任意省都对。配套：① 光柱加 `onPointClick`(点柱=选省)；② 省级面保持**透明**(不糊屏)、选中靠金描边；③ 选中**不抬高**(抬高的面会拦截点向邻省的点击)；④ 点省**不再移动镜头**(每次飞镜头会让下次点击落到错位的省)。
...
### 9. 今日 commit
`99e9f65` 抗竞争推送 → `6e95268` HANDOFF → `fb7a555` 时区+链接解码 → `1a5e46f` 光柱热度 → `f5a3982` 中国省级下钻 → `aa2e2ce`/`9271cef` 修卡死 → `04ec283` 修镜头 → `d5a435e` 修金色糊屏 → `7dac457` 修大省选中糊屏 → `4ee7840` 省份标签栏(修面板挡地图导致的选择卡死)
